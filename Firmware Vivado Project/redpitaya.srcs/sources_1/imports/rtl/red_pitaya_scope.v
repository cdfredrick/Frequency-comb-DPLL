...
if (adc_rstn_i == 1'b0) begin
   ...
   adc_dv      <=  1'b0 ;
    ...
    // JD: integrate-and-dump decimator, set_dec is the decimation ratio
   if ((adc_dec_cnt >= set_dec) || adc_arm_do) begin // start again or arm
    ...
    // JD: adc_data_valid is for the decimated data, adc_a_dat and adc_b_dat
   adc_dv <= (adc_dec_cnt >= set_dec) ;

    // JD: divides the data by a variable power of 2
    // also selects between the filtered or decimated data
    // note that only a handful of values are possible
...
// Write
// JD: this is the control logic for the RAM write pointer, write enable, also saves the trigger location
always @(posedge adc_clk_i) begin
   if (adc_rstn_i == 1'b0) begin
    ...
    // JD: write pointer is always counting when there is new data unless we are resetting
      if (adc_rst_do)
         adc_wp <= {RSZ{1'b0}};
      else if (adc_we && adc_dv)
         adc_wp <= adc_wp + 1;

    // JD: save write pointer at trigger arrival
      if (adc_rst_do)
         adc_wp_trig <= {RSZ{1'b0}};
...
// JD: this implements the actual write port of the dual-port ram which holds the filtered, decimated samples 
always @(posedge adc_clk_i) begin
...
// Read
// JD: this just seems to be a delay line for the system bus read enable or write enable?
// I think I understand now: this is the delay line to match the strobing of sys_ack with the actual data being read out of the ram
// which in fact matches the latency that I noticed in the "always" block just below 
always @(posedge adc_clk_i) begin
   if (adc_rstn_i == 1'b0)
      adc_rval <= 4'h0 ;
   else
      adc_rval <= {adc_rval[2:0], (sys_ren || sys_wen)};    // JD: not sure why sys_wen is there since this is only used for reading I think 
end
assign adc_rd_dv = adc_rval[3];

// JD: this implements the actual read port of the dual-port ram
// this is the read-out port which is connected to the ghetto-system-bus.
// note that the read seems to take a few clock cycles (3 I think?) in addition to whatever latency is added by the system bus
always @(posedge adc_clk_i) begin
...
// JD: this is the code to read out the current state of various registers (and the actual data!)
wire sys_en;
...
if (adc_rstn_i == 1'b0) begin
   ...
   casez (sys_addr[19:0])
     ...
     // direct reads (before the fifo)
     20'h00094 : begin sys_ack <= sys_en;          sys_rdata <= {adc_b_dat[14-1], adc_b_dat[14-1], adc_b_dat, adc_a_dat[14-1], adc_a_dat[14-1], adc_a_dat}        ; end

     20'h1???? : begin sys_ack <= adc_rd_dv;       sys_rdata <= {16'h0, adc_a_rd[14-1], adc_a_rd[14-1], adc_a_rd}              ; end   // JD: changed to sign-extend to 16 bits to make it easier once the values are in the processor
     20'h2???? : begin sys_ack <= adc_rd_dv;       sys_rdata <= {16'h0, adc_b_rd[14-1], adc_b_rd[14-1], adc_b_rd}              ; end
